...
const arrivalTicketInput = document.getElementById('arrival-ticket');
const arrivalPlateInput = document.getElementById('arrival-plate');
...
if (arrivalPlateInput) {
  arrivalPlateInput.addEventListener('blur', () => {
    arrivalPlateInput.value = sanitizePlateValue(arrivalPlateInput.value);
  });
}
...
arrivalForm.addEventListener('submit', async (event) => {
  ...
  const ticketUid = arrivalTicketInput.value.trim();
  const sanitizedPlate = sanitizePlateValue(arrivalPlateInput.value.trim());
  const imageFile = arrivalPhoto.files?.[0];

  if (!ticketUid) {
    arrivalStatus.textContent = 'Ticket UID is required';
    arrivalStatus.classList.add('error');
    return;
  }

  if (!sanitizedPlate || sanitizedPlate.length < 2 || sanitizedPlate.length > 10) {
    arrivalStatus.textContent = 'License plate must be 2-10 letters or numbers';
    arrivalStatus.classList.add('error');
    return;
  }

  arrivalPlateInput.value = sanitizedPlate;

  if (!imageFile) {
    arrivalStatus.textContent = 'Vehicle photo is required';
    arrivalStatus.classList.add('error');
    return;
  }
  ...
  formData.append('plateNumber', sanitizedPlate);
